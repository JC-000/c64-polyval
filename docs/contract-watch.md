...
| c64-x25519 | adopter | **v0.16.0 — RETRACTED (contract#193), twelve open**; PR#141 and PR#146 merged (our #86's and #94's twins), **PR#147 open** against #130 — their public header used `.import` for names an APP_OWNED consumer defines, so such a consumer could not include it at all. Their tag is still v0.16.0, so no fix is in a tag and S4's reading is unmoved | their settling tag, verified at the tag with a §6g positive control: `src/precalc_manifest.s` present, `lib_manifest.s` 0 macro invocations (3 at v0.13.0), ref+path confirmed to exist so the zero is a real zero. Member isolation shipped at v0.14.0; v0.15.0 added §8.2 `A = a` and ABI 3 → 4; v0.16.0 closes their #128 |
