...
|---|---|---|
| **S1** | **NOT MET; nine open, but only in a bookkeeping sense.** Six have fixes in flight on four branches, all adversarially reviewed to convergence: **#87** (stamper), **#89/#94/#103** (composing mode + the §8.4 doc sweep), **#97** (ar65 + the member floor), **#105** (ZP aliasing). Three are DELIBERATE post-release follow-ups, each deferred with recorded reasoning rather than left undone: **#104** (NO_AES arms unlinked in the composing mode), **#106** (hardcoded object count), **#107** (decoy-satisfiable site count). So the release decision is whether S1 means *zero open* or *nothing unfixed that this cycle set out to fix* | **decision point** |
