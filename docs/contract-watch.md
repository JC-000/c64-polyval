...
## 6i. Sibling findings triaged 2026-09-08 — measured N/A, do not refile

Five items landed in siblings within a few hours (nist merged #166/#169/#171,
closed its long-running leg audit #142, and filed #172 and #173; x25519 merged
#149 closing #132). None produced work here, and
each was checked by measurement rather than by reading our docs — the
§6g failure mode is believing a repo is clean because its own prose says so.

| Sibling finding | Verdict here | How it was established |
|---|---|---|
| **nist#159** gated-surface leg reads only the DEFAULT variant arm, so a bare export leaking in a variant arm passes every leg | **Already covered** | `tools/check_composing_mode.py` (PR #111, open) derives all six arms from the Makefile's own target list with a `len != 6` abort, and builds each arm BOTH bare and gated. The hardcoded per-arm expected-value table nist#159 describes is the shape its docstring already records being rejected twice in review. |
| **nist#161** footprint measurand charges pre-segment alignment fill, which the new §5 clause forbids | **N/A, structurally** | `check_footprints.py` sums the per-segment **Size** column of the `ld65 -m` map (`resident = sum(s for _,_,s in hits)`), never a start-to-end extent. Inter-segment fill falls between rows and is never summed. We carry `align = $100` on three table segments, so this was worth measuring rather than assuming. |
| **nist#172** leg 2: a tool prints `FATAL:` and exits 0, so a run that never happened reports success | **N/A, verified** | Every `FATAL:`/`FAIL:` site in `tools/*.py` reaches `sys.exit(1)`; no `sys.exit(0)` sits on a failure path. The shell half — `X || (echo FAIL; exit 1)` in a `;`-chain — appears nowhere in `tools/*.sh` or the Makefile, which is the contract#193 shape CLAUDE.md already names. Leg 1 (firmware note hardcoded to one device) is N/A: no device-attribution tooling here. |
| **x25519#132** an archive member nothing inside the archive imports is dropped by `ld65`'s single scan unless the archive is linked last | **N/A, but for a narrower reason than "§8.1 is N/A"** | Measured over `polyval.a`: **five of ten members are unreachable by intra-archive scan** — `lib_version.o`, `zp_config.o`, `lib_manifest.o`, `precalc_manifest.o` (the isolated §1/§2/§5/§8.4 contract TUs), plus `aes_decrypt.o` and `gcm_siv.o`. That is the same structural property x25519 shipped. It cannot bite here because the importer is always a consumer **object**, which `ld65` links unconditionally regardless of command-line position; the hazard needs a sibling **archive** importing our names, and we neither provide nor defer a §8.1–§8.3 shared primitive. |
| **nist#173** `measure_mhz` reports a fixed ~2-jiffy overhead as a constant 6.25% clock deficit at every setting — the constancy is the tell | **N/A, and the class is already handled** | No device-clock provenance exists here: cycle figures come from exact VICE cycle counting, not a jiffy-sampled wall-clock window. The portable *class* — a fixed overhead reported as a proportional one — is what `tools/benchmark_polyval.py` already avoids: it measures the calibration overhead directly and aborts unless it lands in 10–25 cycles (`FATAL: calibration overhead = {overhead} cycles`), which is the two-point discipline nist#173 asks for rather than the assume-and-dilute one. CLAUDE.md's "re-measure before quoting a cycle count" covers the reporting half. |

**The x25519#132 row is a standing condition, not a closed one.** The
five unreachable members are real; only the absence of a provider surface
keeps them safe. If this library ever exports a primitive a sibling archive
imports, that N/A expires and the member-extraction order becomes live.
Recorded here rather than filed, per the churn test — an issue against a
capability we do not have and explicitly disclaim (API.md §9.3) would be
work whose only product is closing its own loop.

**Fixture note.** The member-reachability measurement above returned
"0 distinct imports across 10 members" on its first run — impossible, since
`gcm_siv.o` imports 29 names. Cause: the loop was written for `bash` word
splitting and run under `zsh`, which does not split unquoted `$M`, so the
body executed once with the whole list as one word. The all-zero result
would have read as "no member is pulled by any sibling", i.e. a *scarier*
finding than the truth, and it was wrong. Believe a reachability zero only
after something in the same run is non-zero.
