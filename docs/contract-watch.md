...
|---|---|---|
| **S1** | **NOT MET; six open.** Ten filed by this session's audit, four more found while fixing them; **#85, #86, #90, #91, #96, #99 closed** by PR#88, PR#92, PR#98 and PR#100, all merged 2026-09-07 and all adversarially reviewed, every one of which had its headline claim broken by that review. Open: **#87, #89, #93, #94, #95, #97** — #95 has a fix on `fix/95-verify-footprints` in review. Verified on each merged tree, not assumed: `make verify` passes nine targets and the artifact guard still exits 2 on a tracked `.o` | **improving** |
...
| c64-nist-curves | adopter | **v0.14.0 — tagged with open findings; three open** (#142, #159 variant-arm gap, #161 pre-segment fill). PR#160 merged closing #158 (our #90's twin); **PR#162 merged 2026-09-07 closing #155** [HIGH] §6.1 — but **still on `main`, tag unchanged at v0.14.0**, so their settling tag does not carry either fix and S4's reading is unmoved | #153 and #154 both closed. #154 verified at the tag with a §6g positive control: bare `zp_*` aliases in `zp_config.s` went **4 → 0**, and `src/zp_aliases.s` (104 lines) plus `src/mul_aliases.s` appeared as the separate archived TUs. #153 is §8.2 REU — **N/A to this library** and their domain to certify; recorded as closed by them, not audited here |
| c64-x25519 | adopter | **v0.16.0 — RETRACTED (contract#193), twelve open** (#130, #132, #134–#138, #140, #142–#145). **PR#146 merged 2026-09-07 closing #139** — no target ever built `LIB_NO_BARE_EXPORTS`, **our #94's twin, and now a merged reference implementation to read before writing ours**. PR#141 merged closing #133 (our #86's twin). Both fixes are on `main`; their tag is still v0.16.0 | their settling tag, verified at the tag with a §6g positive control: `src/precalc_manifest.s` present, `lib_manifest.s` 0 macro invocations (3 at v0.13.0), ref+path confirmed to exist so the zero is a real zero. Member isolation shipped at v0.14.0; v0.15.0 added §8.2 `A = a` and ABI 3 → 4; v0.16.0 closes their #128 |
